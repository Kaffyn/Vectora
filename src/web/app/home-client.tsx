...
export default function HomeClient({
  ...
  React.useEffect(() => {
    const loadSessions = async () => {
      try {
        ...
        if (!res.ok) return;
        const data: Array<{id:string;title:string;messages:any[];updated_at:string}> = await res.json();
        if (!data || data.length === 0) return;
        const loaded: ChatSession[] = data.map(c => ({
          id: c.id,
  ...
  // Sincronização Bidirecional do Modelo RAG na Montagem
  React.useEffect(() => {
  ...
  const handleGeminiKeyChange = (key: string) => {
    setGeminiKey(key);
    // Não salvamos a cada tecla, talvez usar um debounce ou botão de salvar? 
    // Por enquanto, salvamos pra garantir persistência imediata.
  ...
  const handleLocaleChange = async (newLocale: Locale) => {
    setLocale(newLocale);
    document.cookie = `vectora_locale=${newLocale}; Max-Age=31536000; Path=/; SameSite=Lax`;
    try {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      fetch(`${API}/conversations/${currentSessionId}/messages`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: 'ai', content: aiMsg.content })
      }).catch(() => {});
    } catch (err: any) {
      const errorMsg: ChatMessage = {
        id: (Date.now() + 1).toString(), role: "ai",
        content: err.message || "O Motor RAG em Go está offline. Inicie o servidor Go na porta 8080.",
        sources: [],
      };
      setSessions(prev => prev.map(s => s.id === currentSessionId 
