...
- **Valkey (Cache):** Armazena dados frequentemente acessados em memória. Em Docker, um container próprio é usado. No modo standalone local, o sistema oferece um *fallback* (em memória/arquivo via pickle/json) caso um servidor Valkey/Redis não esteja configurado.
...
3. **Dependências Opcionais:** Através de *extras* do Python no gerenciador de pacotes, os usuários instalam apenas o que precisam (ex: `pip install "vectora[qdrant]"` ou `vectora[full]`).
