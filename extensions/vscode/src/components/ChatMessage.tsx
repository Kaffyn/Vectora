                            ...
                            pre: ({ node, ...props }) => (
                                <div className="relative group my-4 rounded-lg overflow-hidden border border-vscode-widget-border shadow-sm">
                                    <div className="flex items-center justify-between px-3 py-1 bg-vscode-editorWidget-background border-b border-vscode-widget-border">
                                        <span className="text-[9px] font-bold text-vscode-descriptionForeground uppercase tracking-tighter">Code</span>
                                    </div>
                                    <pre {...props} className="bg-vscode-editor-background p-4 overflow-x-auto font-mono text-[11px] leading-relaxed scrollbar-thin" />
                                </div>
                            ),
                            code: ({ node, ...props }) => {
                                const isInline = !props.className;
                                return isInline ? (
                                    <code className="bg-vscode-editor-selectionBackground/30 px-1 py-0.5 rounded text-vscode-editor-foreground font-mono text-[12px]" {...props} />
                                ) : (
                                    <code {...props} />
                                );
                            },
...
const ToolBadge: React.FC<{ tool: ToolCall }> = ({ tool }) => {
    const getIcon = () => {
        switch (tool.status) {
            case 'completed': return <CheckCircle2 size={11} className="text-vscode-testing-iconPassed" />;
            case 'in_progress': return <Loader2 size={11} className="animate-spin text-vscode-button-background" />;
            case 'failed': return <AlertCircle size={11} className="text-vscode-testing-iconErrored" />;
        }
    };
