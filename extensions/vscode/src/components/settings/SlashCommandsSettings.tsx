        ...
        <div key={`${command.source}-${command.name}`} className="p-2.5 px-2 rounded-xl border border-transparent">
          <div className="flex items-start justify-between gap-2 flex-col min-[400px]:flex-row overflow-hidden">
            <div className="flex-1 min-w-0">
              ...
              {/* Command description */}
              {command.description && (
                <div className="text-xs text-vscode-descriptionForeground mt-1 line-clamp-3">{command.description}</div>
              )}
            </div>
          ...
          {hasWorkspace && (
            <>
              <div className="flex items-center gap-2 px-2 py-2 mt-2 cursor-default">
                ...
                <span className="font-medium text-lg">{t("settings:slashCommands.workspaceCommands")}</span>
              </div>
