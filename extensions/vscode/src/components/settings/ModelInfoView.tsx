...
export const ModelInfoView = ({
  ...
  const priceInfoItems = [
    modelInfo?.inputPrice !== undefined && (
      <>
        <span className="font-medium">{t("settings:modelInfo.inputPrice")}:</span> {formatPrice(modelInfo.inputPrice)} /
        1M tokens
      </>
    ),
    modelInfo?.outputPrice !== undefined && (
      <>
        <span className="font-medium">{t("settings:modelInfo.outputPrice")}:</span> {formatPrice(modelInfo.outputPrice)}{" "}
        / 1M tokens
      </>
    ),
              ...
              <tbody>
                ...
                {allowedTierNames.includes("flex") && (
                  <tr className="border-t border-vscode-dropdown-border/60">
                    ...
                    <td className="px-3 py-1.5 text-right">
                      {fmt(modelInfo?.tiers?.find((t) => t.name === "flex")?.inputPrice ?? modelInfo?.inputPrice)}
                    </td>
                    <td className="px-3 py-1.5 text-right">
                      {fmt(modelInfo?.tiers?.find((t) => t.name === "flex")?.outputPrice ?? modelInfo?.outputPrice)}
                    </td>
                    <td className="px-3 py-1.5 text-right">
                      {fmt(
                        modelInfo?.tiers?.find((t) => t.name === "flex")?.cacheReadsPrice ?? modelInfo?.cacheReadsPrice,
                      )}
                    </td>
                  </tr>
                )}
                {allowedTierNames.includes("priority") && (
                  <tr className="border-t border-vscode-dropdown-border/60">
                    ...
                    <td className="px-3 py-1.5 text-right">
                      {fmt(modelInfo?.tiers?.find((t) => t.name === "priority")?.inputPrice ?? modelInfo?.inputPrice)}
                    </td>
                    <td className="px-3 py-1.5 text-right">
                      {fmt(modelInfo?.tiers?.find((t) => t.name === "priority")?.outputPrice ?? modelInfo?.outputPrice)}
                    </td>
                    <td className="px-3 py-1.5 text-right">
                      {fmt(
